# Contributing to AnthraSpace

Thanks for contributing to AnthraSpace.

## Before You Start

- Keep changes scoped to a clear user-facing improvement, bug fix, or refactor.
- AnthraSpace targets macOS, Linux, and Windows. Every change must stay compatible with all three platforms unless the code is explicitly guarded by a runtime platform check.
...
- AnthraSpace must work against local repositories, remote servers, and SSH worktrees. Do not assume a process, file, credential, shell, or network path exists only on the local machine.
- AnthraSpace supports many CLI agents, integrations, and git providers. Keep generic behavior provider-neutral; guard integration-specific logic behind explicit checks.
...
- **Include your X (Twitter) handle!** We love giving shoutouts to our contributors when we merge features on [@anthraspace_build](https://x.com/anthraspace_build).
