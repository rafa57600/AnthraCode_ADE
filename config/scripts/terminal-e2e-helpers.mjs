#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running AnthraSpace
 ...
 *
 * Usage:
 *   import { AnthraSpaceTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new AnthraSpaceTerminal(9444)       // CDP port
 *   await term.connect()
...
export class AnthraSpaceTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching AnthraSpace */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = '/tmp/anthraspace-pty-probe.png') {
    for (let i = 1; i <= maxId; i++) {
      evalInRenderer(
        this.port,
  ...
  /**
   * Take a screenshot of the AnthraSpace window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = '/tmp/anthraspace-terminal.png') {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in AnthraSpace.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot('/tmp/anthraspace-lang-check.png')
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new AnthraSpaceTerminal(port)
  console.log('Connecting to AnthraSpace on CDP port', port, '...')
  term.connect()
