...
REM launcher resolves APP_DIR back to resources/ and `anthraspace open` cannot find
REM AnthraSpace.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\AnthraSpace.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate AnthraSpace.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: AnthraSpace packages the CLI entrypoint outside app.asar so the public shell
