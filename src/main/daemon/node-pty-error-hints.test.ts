...
const PTY_ALLOCATION_HINT = [
  ...
  '',
  'AnthraSpace requires a pty device to launch a new terminal. This error is usually due to having too many terminal windows or terminal sessions open, either in AnthraSpace or another program.',
  '',
...
describe('node-pty diagnostic error hints', () => {
  it('parses the native step and errno without dropping the original message', () => {
    ...
    expect(addNodePtyRecoveryHint(message)).toBe(
      `Daemon's node-pty install is gone (worktree deleted?). Restart AnthraSpace. ${message}`
    )
  })
