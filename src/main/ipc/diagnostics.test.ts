...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { ANTHRASPACE_BUILD_IDENTITY?: unknown }).ANTHRASPACE_BUILD_IDENTITY
    delete (globalThis as { ANTHRASPACE_DIAGNOSTICS_TOKEN_URL?: unknown })
      .ANTHRASPACE_DIAGNOSTICS_TOKEN_URL
    process.env.ANTHRASPACE_DIAGNOSTICS_TOKEN_URL =
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      ANTHRASPACE_BUILD_IDENTITY?: 'stable'
      ANTHRASPACE_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.ANTHRASPACE_BUILD_IDENTITY = 'stable'
    globalOverrides.ANTHRASPACE_DIAGNOSTICS_TOKEN_URL =
      'https://official.example.com/diagnostics/token'
    process.env.ANTHRASPACE_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
