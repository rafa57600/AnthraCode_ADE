...
function getOpenCodePluginSource(): string {
  ...
  return [
    ...
    '  // ANTHRACODE: AnthraCode reuses the OpenCode-compatible plugin API but',
    '  // must report its own provider identity to Orca status hooks.',
    '  return process.env.ORCA_TUI_AGENT_TYPE === "anthracode" ? "anthracode" : "opencode";',
    '}',
    '',
    'let lastStatus = "idle";',
    ...
    '  const url = `http://127.0.0.1:${coords.port}/hook/${getHookSource()}`;',
