...
function buildAtomFeed(tags: string[]): string {
  const entries = tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/rafa57600/AnthraSpace/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/rafa57600/AnthraSpace/releases.atom') {
  ...
  it.each([
    ...
    async (platform, manifestName) => {
      ...
      expect(manifestUrls).toEqual([
        `https://github.com/rafa57600/AnthraSpace/releases/download/v1.4.1/${manifestName}`
      ])
      expect(assetUrls).toEqual([
        'https://github.com/rafa57600/AnthraSpace/releases/download/v1.4.1/Orca-1.4.1-arm64-mac.zip'
      ])
    }
  )
