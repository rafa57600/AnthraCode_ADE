...
function formatBrowserTabUrlLabel(url: string): string {
  if (url === ANTHRASPACE_BROWSER_BLANK_URL || url === 'about:blank') {
    return 'New Tab'
  }
  try {
...
export function getBrowserTabLabel(tab: BrowserTabState): string {
  if (
    !tab.title ||
    tab.title === tab.url ||
    tab.title === ANTHRASPACE_BROWSER_BLANK_URL ||
...
function isBlankBrowserTab(tab: BrowserTabState): boolean {
  return tab.url === ANTHRASPACE_BROWSER_BLANK_URL || tab.url === 'about:blank'
}
