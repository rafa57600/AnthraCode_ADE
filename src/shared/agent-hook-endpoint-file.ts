...
export function parseAgentHookEndpointFile(contents: string): AgentHookEndpoint {
  ...
  if (
    !values.ANTHRASPACE_AGENT_HOOK_PORT ||
    !values.ANTHRASPACE_AGENT_HOOK_TOKEN ||
    !values.ANTHRASPACE_AGENT_HOOK_ENV ||
    !values.ANTHRASPACE_AGENT_HOOK_VERSION
  ) {
    throw new Error('Agent hook endpoint file is missing required fields')
  }
  return {
    port: values.ANTHRASPACE_AGENT_HOOK_PORT,
    token: values.ANTHRASPACE_AGENT_HOOK_TOKEN,
    env: values.ANTHRASPACE_AGENT_HOOK_ENV,
    version: values.ANTHRASPACE_AGENT_HOOK_VERSION
  }
}
