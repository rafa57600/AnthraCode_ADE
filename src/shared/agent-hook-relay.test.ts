...
describe('isRemoteAgentHooksEnabled', () => {
  ...
  it('is off for empty / "0"', () => {
    expect(isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: '' })).toBe(
      false
    )
    expect(isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: '0' })).toBe(
      false
    )
    expect(isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: '   ' })).toBe(
      false
  ...
  it('is on for any other non-empty value', () => {
    expect(isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: '1' })).toBe(
      true
    )
    expect(isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: 'on' })).toBe(
      true
    )
    expect(
      isRemoteAgentHooksEnabled({ [ANTHRASPACE_FEATURE_REMOTE_AGENT_HOOKS_ENV]: 'true' })
    ).toBe(true)
  })
})
