...
export function remoteRuntimeUnavailableError(
  message = 'Remote AnthraSpace runtime closed the connection.'
...
export function parseAuthenticatedFrame(plaintext: string): RemoteRuntimeClientError | null {
  ...
  return new RemoteRuntimeClientError(code, 'Remote AnthraSpace runtime rejected the pairing token.')
}
